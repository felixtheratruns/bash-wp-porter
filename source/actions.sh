...
doActions(){
    ...
    rot mkdir -p "$local_uploads_path"
...
#       done
    ...
    if $(isInOpts "opt_arr[@]" 'all'); then
        ...
        #echo scp_copy "src_scp_args $src_scp_args" "src_uploads_path $src_uploads_path" "dest_scp_args $dest_scp_args" "dest_uploads_path $dest_uploads_path" "-r" "*" "src_ssh_port $src_ssh_port" "dest_ssh_port $dest_ssh_port" "local_uploads_path $local_uploads_path"
        ...
        git_pull "$dest_branch" "$dest_path" "$dest_scp_args" "$dest_ssh_port" 
    fi
    ...
    if [[ $local_uploads_path == *uploads ]] && [[ -e "$local_uploads_path/"* ]]; then
        rot echo "removing uploads files"
        rot rm -r "$local_uploads_path/"* 
    fi
}
